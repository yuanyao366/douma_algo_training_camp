...
var maxCoins = function(nums) {
    ...
    const numsTemp = new Array(nums.length + 2).fill(0)
    for (let i = 1; i <= nums.length; i++) numsTemp[i] = nums[i - 1]
    numsTemp[0] = 1
    numsTemp[nums.length + 1] = 1

    // 创建 dp 数组，并且 dp 数组中的每个元素都初始化成 0
    const length = numsTemp.length
    ...
    for (let i = length - 2; i >= 0; i--) {
        ...
        for (let j = i + 2; j < length; j++) {
            ...
            for (let k = i + 1; k < j; k++) {
                ...
                const temp = dp[i][k] + dp[k][j] + numsTemp[i] * numsTemp[k] * numsTemp[j]
